...
Credential and runtime-secret values are never intended to be committed:
...
`CLOUDFLARE_ACCOUNT_ID` and the D1 database UUID are deployment identifiers rather than authentication secrets, but the production workflow still resolves the D1 UUID dynamically instead of baking account-specific state into the repository.
