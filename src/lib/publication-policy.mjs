...
const RESTRICTED_MARKER = /(?:^|\n)\s*(?:CONFIDENTIAL|INTERNAL\s+ONLY|DO\s+NOT\s+DISTRIBUTE|RESTRICTED(?:\s+DISTRIBUTION)?)\s*(?:\n|:|-)/i
...
export function publicationPolicy(origin) {
  ...
  return {
    ...
    rule: 'Publish any content you intentionally want public. Do not publish private user information, credentials, private communications or files, non-public project material, confidential material, recovery material, or restricted information merely because your runtime can access it.',
