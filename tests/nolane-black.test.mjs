...
test('Nolane Black neutralizes remaining sci-fi and decorative legacy surfaces', () => {
  ...
  assert.match(css, /\.section-label[^}]*text-transform:\s*none/is)
  ...
  assert.match(css, /\.soft-chip[^}]*border-radius:\s*5px/is)
...
test('interactive states cannot reintroduce AI color or glass material', () => {
  const css = read('public/nolane-black.css')
  assert.match(css, /\.topbar\s*\{[^}]*backdrop-filter:\s*none/is)
  assert.match(css, /\.mobile-nav\s*\{[^}]*backdrop-filter:\s*none[^}]*box-shadow:\s*none/is)
  assert.match(css, /\.topic-shelf a\s*\{[^}]*border-radius:\s*7px[^}]*background:\s*#050505/is)
  assert.match(css, /\.chatgpt-live-dot\[data-state="online"\][^}]*box-shadow:\s*none/is)
  assert.match(css, /\.chatgpt-live-dot\[data-state="degraded"\][^}]*box-shadow:\s*none/is)
  assert.match(css, /\.chatgpt-copy-button:hover\s*\{[^}]*background:\s*#e5e5e5/is)
  assert.match(css, /\.chatgpt-copy-button:focus-visible[^}]*outline-color:\s*#ffffff/is)
  assert.match(css, /\.chatgpt-connect-card:focus-visible[^}]*outline-color:\s*#ffffff/is)
})
